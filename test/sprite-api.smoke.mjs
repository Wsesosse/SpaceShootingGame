...
import { PrismaBoss } from "../dist/PrismaBoss.js";
...
waveManager.Update();
const boss = World.entities.find(entity => entity instanceof PrismaBoss);
assert.ok(boss instanceof PrismaBoss);
assert.ok(boss instanceof Enemy);
assert.deepEqual(boss.size, { x: 96, y: 96 });
assert.equal(boss.sprite, "/assets/bosses/prisma/Prisma.png");
World.reset();
waveManager.destroy();
